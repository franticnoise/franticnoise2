...
export function calcMs(speed) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    !document.getElementById(i)
      ...
      : resultsList.replaceChild(createResultRow, document.getElementById(i));
...
export function gumroadScripts() {
  ...
  script.src =
    "https://assets.gumroad.com/packs/js/overlay-499efd442e6f54649ee6.js";
  script.charset = "utf-8";
  document.head.appendChild(script);
  document.head.innerHTML +=
    '<link rel="stylesheet" href="https://assets.gumroad.com/packs/css/overlay-9325a7da.css" media="screen" />';
}
...
export async function loadJson(section, type) {
  ...
  if (section === "presets") {
    ...
    types.forEach((element) => {
      ...
      element === type
        ? (createTypeDiv.className = "js-synthtypes js-stypes_slctd")
        : (createTypeDiv.className = "js-synthtypes");
      ...
      /// PARSE AND CREATE SYNTH TYPE TOOLTIPS
      let createTypeTooltip = document.createElement("p");
      createTypeTooltip.id = element + "_tooltip";
      createTypeTooltip.className = "p-tooltip-text";
      gumroadSynthType.appendChild(createTypeTooltip);

      const target = document.getElementById(element);
      const tooltip = document.getElementById(element + "_tooltip");

      json.tooltips.forEach((tooltipjson) => {
      if (element === tooltipjson.synth) { tooltip.innerHTML = tooltipjson.tooltip;}
      });
    ...
    /////////////// ADD 'VIEW ALL SYNTH' TAG / LInk console.log(`total banks: ${total_banks}`);
    const gumroadSynthType = document.getElementById("container_types");
    let createAllSynthDiv = document.createElement("div");
    createAllSynthDiv.id = "ALL";
    type === "ALL"
      ? (createAllSynthDiv.className = "js-synthtypes js-stypes_slctd")
      : (createAllSynthDiv.className = "js-synthtypes");
    gumroadSynthType.appendChild(createAllSynthDiv);
    document.getElementById("ALL").innerHTML = 
    `<a href="../index.html?page=presets&type=ALL" target="_parent">ALL BANKS</a> (${total_banks})`;
  }
